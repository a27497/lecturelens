...
final class SubtitleTranslationValidators {

    private static final int MAX_TASK_ID_LENGTH = 64;
    private static final int MAX_LANGUAGE_LENGTH = 32;

    private SubtitleTranslationValidators() {
    }

    static ValidatedTranslationCommand validateCommand(TranslateSubtitleCommand command) {
        if (command == null) {
            throw validationFailure("Subtitle translation command is required");
        }
        ...
        return new ValidatedTranslationCommand(
            taskId,
            command.userId(),
            sourceLanguage,
            targetLanguage,
    ...
    static ValidatedTranslationScope validateScope(String taskId, Long userId, String targetLanguage) {
        String normalizedTaskId = requiredText(taskId, "Task ID is required");
        if (normalizedTaskId.length() > MAX_TASK_ID_LENGTH) {
            throw validationFailure("Task ID is invalid");
        }
        validateUserId(userId);
        return new ValidatedTranslationScope(
            normalizedTaskId,
            userId,
