...
BB 引用修复：受限成本应用包含的固定方法代价必须引用模型选择的共享方法来源及场景来源。概念题独有来源不自动传入，逐题语义复核仍检验这些实际引用能否支持结论。368项机制测试通过。BA开发语义分数已由16/16更正为13/16，正式评测未启动，详见[审阅更正](../eval/l22-completion/citation-review-errata.json)。
